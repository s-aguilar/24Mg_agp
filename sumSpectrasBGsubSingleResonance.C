// Sum all the energy calibrated BG subtracted spectra per detector to address
// referee comment to check for other higher energy gamma-ray transitions

#include <iostream>
#include <list>

void sumSpectrasBGsubSingleResonance(){

    std::list<int> listOfRuns({206,207,208,324,325,
                            326,327,328,329,330});

    const char *detect;
    const char *files;

    TFile *ff = new TFile("E_cal_spectras/summedSpectrasBGsubSingleResonance.root","RECREATE");
	...
	for (int val : listOfRuns){
        ...
        // Open file
        TFile *fyield = new TFile(Form("E_cal_spectras/bgSubtracted_run_%s.root",runNum_String),"READ");
