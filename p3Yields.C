...
void peakFitter(const char *fileName, const char *fileBack, const char *detector,
	...
	h3->GetXaxis()->SetRangeUser(1900,3000);

	P3Peak = single_gauss_area(2212,h3);
	// P3Peak = single_gauss_area_noBG(2212,h3);
	...
	c0->SaveAs(Form("Yields/P3/run0%s/det_%s_Fit.png",runNum.c_str(),detNum.c_str()));
	c0->SaveAs(Form("Yields/P3/det-%i/run0%s_Fit.png",detLoop,runNum.c_str()));

	ofstream myfile;
	myfile.open ("Yields/P3/_P3.csv",std::ios::app);
...
void p3Yields(){
	...
	ofstream myfile;
	myfile.open ("Yields/P3/_P3.csv",std::ios::out);
	...
	for(int ii = 0; ii<13; ii++){
		try {
			gSystem->Exec(Form("mkdir Yields/P3/det-%i",ii));
		}catch(...){}
	}
	...
	for(int i=160;i<upToRun;i++){
		...
		try {
			gSystem->Exec(Form("mkdir Yields/P3/run0%d",i));
		}catch(...){}
		...
		for(int j=0;j<13;j++){ // 13
			...
			peakFitter(files,fileBackground,detect,peak1BackLow,peak1BackHigh,
				...
				peak2SpecLow,peak2SpecHigh,P3-50,P3+40,j);
		}
