...
int peakFitter(const char *fileName,const char *fileBack,const char *detector,double low,double high,int detectorLoop){
	...
	if(runTime0 > 500){
		...
		backfileREAD.open("Yields/P1/_backgroundPeakRanges.csv",std::ios::in);
		double bgPeaks[4][13];
		...
		rangefileREAD.open("Yields/P1/_ranges/peakRanges.csv",std::ios::in);
		...
		// Find peak positions to calibrate to energy
		...
		// Draw results
		hyield->Draw();
		hyield->GetXaxis()->SetRangeUser(0,1300);
		...
		ysubtracted->Draw();
		ysubtracted->GetXaxis()->SetRangeUser(0,1300);
		gPad->SetLogy();
	}
	else{
	...
	// At this point, yusbtracted histogram has or has not been BG subtracted \
		now we fit the peak of interest:									  \
			P2 : single gaussian
	ysubtracted->GetXaxis()->SetRangeUser(1300,1500);
	ysubtracted->Draw();
	gPad->SetLogy();
	ysubtracted->SetStats(kFALSE);

	vector < double > peak;
	peak = iterative_single_gauss_peak(low,high,ysubtracted);

	// ysubtracted->GetXaxis()->SetRangeUser(600,1300);

	double pos = peak[0];
	double pos_err = peak[3];
	double otherPeak = peak[4];
...
void peakID(){
	...
	// 	gSystem->Exec(Form("mkdir Yields/P1/_ranges"));
	// }catch(...){}
	...
	backfileOUT.open ("Yields/P1/_backgroundPeakRanges.csv",std::ios::out);
	...
	rangefileOUT.open ("Yields/P1/_ranges/peakRanges.csv",std::ios::out);
	...
	for(int i=159;i<160;i++){ // 410
		...
		for(int j=0;j<1;j++){
			...
			double peakPos[] = {1246,836.3,850.7,838.1,815.1,859.8,844.8,851.2,839.2,849.2,850.3,837.7,839.2};
			p1 = peakPos[j];


			// Perform peak fitting

			test = peakFitter(files,fileBackground,detect,p1-50,p1+50,j);
		}

		// if (test==0){
		// 	// Write out new spectra peak ranges to gain match to
		// 	ofstream rangefile;
		// 	rangefile.open ("Yields/P1/_ranges/peakRanges.csv",std::ios::out);
