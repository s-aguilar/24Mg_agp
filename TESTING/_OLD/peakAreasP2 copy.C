...
void peakFitter(const char *fileName,const char *fileBack,const char *detector,int low,int high,int detectorLoop){
	...
	if(runTime0 > 500){
		...
		backfileREAD.open("peakAreas/P2/_backgroundPeakRanges.csv",std::ios::in);
		int bgPeaks[4][13];
		...
		rangefileREAD.open("peakAreas/P2/_ranges/peakRanges.csv",std::ios::in);
		int runPeaks[4][13];
	...
	ofstream myfile;
	myfile.open ("peakAreas/P2/_P2.csv",std::ios::app);
...
void p2Yields(){
	...
	try {
		...
		gSystem->Exec(Form("mkdir peakYields"));
		gSystem->Exec(Form("mkdir peakYields/P2"));
		gSystem->Exec(Form("mkdir peakYields/P2/_ranges"));
	}catch(...){}
	...
	ofstream myfile;
	myfile.open ("peakYields/P2/_P2.csv",std::ios::out);
	...
	ofstream backfileIN;
	backfileIN.open ("peakYields/P2/_backgroundPeakRanges.csv",std::ios::out);
	...
	rangefileIN.open ("peakYields/P2/_ranges/peakRanges.csv",std::ios::out);
	...
	for(int i=159;i<162;i++){ // 410
		...
		try {
			gSystem->Exec(Form("mkdir peakYields/P2/run0%d",i));
		}catch(...){}
		...
		rangefileOUT.open ("peakYields/P2/_ranges/peakRanges.csv",std::ios::out);
		for(int ii = 0;ii<13;ii++){
