...
for ch in channels:
    ...
    for T in temperature:
        ...
        if ch == 'p0':
            ...
            if ch == 'p0':
                ...
                plt.plot(E,integrand,c='g',label='integrand p0')
                # plt.plot(EE,mB,c='b',label='MB')
                # plt.scatter(E,cross,c='b')
                plt.xlabel('E_CM (MeV)',fontsize=14)
                # plt.ylabel('Angle Integrated Cross-Section (barns)', fontsize=14)
                ...
                cross1 = dict_channels['p1']['Fit_Cm_Cross'].values    # angle integrated cross
                E1 = dict_channels['p1']['E_Cm'].values        # E_Cm
                integrand1 = cross1*np.exp(-11.604*E1/T)*E1
                plt.plot(E1,integrand1,c='r',label='integrand p1')

                cross2 = dict_channels['p2']['Fit_Cm_Cross'].values    # angle integrated cross
                E2 = dict_channels['p2']['E_Cm'].values        # E_Cm
                integrand2 = cross2*np.exp(-11.604*E2/T)*E2
                plt.plot(E2,integrand2,c='y',label='integrand p2')


                plt.legend()
                filenames.append('blah/Overlay_at_%fGK.png'%T)
    ...
    imageio.mimsave('blah/integrand.gif', images,duration=0.5)
    ...
    plt.plot(temperature,rxnRate,label='%s-angInt'%ch,color='coral')
    plt.legend()
    plt.yscale('log')
    plt.xlim(0,10)
    plt.ylim(1e-20,1e8)
    plt.ylabel('Fine Reaction Rate (cm$^3$ mol$^{-1}$ s$^{-1}$)',fontsize=14)
    plt.xlabel('Temperature (T9)',fontsize=14)
    plt.savefig('%s_ReactionRate_azureAngleIntegratedFiner10.png'%ch,dpi=900)
    ...
    if ch == 'p0':
        ...
        plt.xlabel('Temperature (T9)',fontsize=14)
        plt.title('p$_{0}$ Finer Reaction Rates Ratio',fontsize=20)
        eq = r'$\frac{Rate~p_{{0}_{angInt}}}{Rate~p_{0}}$'
        plt.text(9, 1, eq, {'color': 'k', 'fontsize': 18}, va="top", ha="right")
        # plt.grid(b=True, which='both', axis='both')

        savePath = os.path.join(desiredDir,'p0_RatioReactionRates_azureAngleIntegratedFiner10.png')
