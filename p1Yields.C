...
int peakFitter(const char *fileName,const char *fileBack,const char *detector,double low,double high,int detectorLoop){
	...
	if(runTime0 > 500){
		...
		// Draw results
		// hyield->Draw();
		hyield->GetXaxis()->SetRangeUser(600,1200); // 600,1200
	...
	p1Peak = double_gauss_area(843.76,867.7,h3);
...
void p1Yields(){
	...
	double peak1BackLow[] = {200,210,220,210,209,226,220,220,215,215,220,210,215};
	double peak1BackHigh[] = {235,250,264,260,250,267,260,260,250,255,260,250,250};
	...
	for(int i=159;i<162;i++){ // 410
		...
		for(int j=0;j<13;j++){
			...
			test = peakFitter(files,fileBackground,detect,p1-40,p1+90,j);
		}
