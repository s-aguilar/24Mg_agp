...
void peakFitter(const char* fileName,const char* detector,int low, int high){
	...
	cout << ysubtracted->GetNbinsX() << endl;
	...
	ofstream myfile;
	myfile.open ("peakAreas.csv",std::ios::app);
	myfile << Form("run0%s",runNum.c_str()) <<","<< Form("det_%s",detNum.c_str())<<","<< area <<","<< err << "\n";
	myfile.close();
