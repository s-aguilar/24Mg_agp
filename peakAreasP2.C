...
void peakFitter(const char *fileName,const char *fileBack,const char *detector,int low,int high,int detectorLoop){
	...
	if(runTime0 > 500){
		...
		ifstream backfile;
		backfile.open("peakAreas/P2/_backgroundPeakRanges.csv",std::ios::in);
		int bgPeaks[4][13];
		...
		ifstream rangefile;
		rangefile.open("peakAreas/P2/_ranges/peakRanges.csv",std::ios::in);
		int runPeaks[4][13];
	...
	c0->SaveAs(Form("peakAreas/P2/run0%s/det_%s_Fit.png",runNum.c_str(),detNum.c_str()));
...
void peakAreasP2(){
	...
	try {
		...
		gSystem->Exec(Form("mkdir peakAreas"));
		gSystem->Exec(Form("mkdir peakAreas/P2"));
		gSystem->Exec(Form("mkdir peakAreas/P2/_ranges"));
	}catch(...){}
	...
	ofstream myfile;
	myfile.open ("peakAreas/P2/_P2.csv",std::ios::out);
	...
	ofstream backfile;
	backfile.open ("peakAreas/P2/_backgroundPeakRanges.csv",std::ios::out);
	...
	for(int ii = 0;ii<13;ii++){
		backfile<<peak1BackLow[ii]<<",";
	}backfile<<endl;
	for(int ii = 0;ii<13;ii++){
		backfile<<peak1BackHigh[ii]<<",";
	}backfile<<endl;
	for(int ii = 0;ii<13;ii++){
		backfile<<peak2BackLow[ii]<<",";
	}backfile<<endl;
	for(int ii = 0;ii<13;ii++){
		backfile<<peak2BackHigh[ii]<<",";
	}backfile<<endl;
	backfile.close();
	...
	ofstream rangefile;
	rangefile.open ("peakAreas/P2/_ranges/peakRanges.csv",std::ios::out);
	...
	for(int ii = 0;ii<13;ii++){
		rangefile<<peak1SpecLow[ii]<<",";
	}rangefile<<endl;
	for(int ii = 0;ii<13;ii++){
		rangefile<<peak1SpecHigh[ii]<<",";
	}rangefile<<endl;
	for(int ii = 0;ii<13;ii++){
		rangefile<<peak2SpecLow[ii]<<",";
	}rangefile<<endl;
	for(int ii = 0;ii<13;ii++){
		rangefile<<peak2SpecHigh[ii]<<",";
	}rangefile<<endl;
	rangefile.close();
	...
	for(int i=159;i<162;i++){ // 410
		...
		try {
			gSystem->Exec(Form("mkdir peakAreas/P2/run0%d",i));
		}catch(...){}
		...
		for(int j=0;j<13;j++){
			...
			// cout << _peak1SpecHigh[j] << endl;

			// for(int ii = 0;ii<13;ii++) cout << _peak1SpecLow[ii] << endl;

			// // Write out new peak ranges
			// ofstream rangefile;
			// rangefile.open ("peakAreas/P2/_ranges/peakRanges.csv",std::ios::out);
			// for(int ii = 0;ii<13;ii++){
			// 	rangefile<<_peak1SpecLow[ii]<<",";
			// }rangefile<<endl;
			// for(int ii = 0;ii<13;ii++){
			// 	rangefile<<_peak1SpecHigh[ii]<<",";
			// }rangefile<<endl;
			// for(int ii = 0;ii<13;ii++){
			// 	rangefile<<_peak2SpecLow[ii]<<",";
			// }rangefile<<endl;
			// for(int ii = 0;ii<13;ii++){
			// 	rangefile<<_peak2SpecHigh[ii]<<",";
			// }rangefile<<endl;
			// rangefile.close();
		}
